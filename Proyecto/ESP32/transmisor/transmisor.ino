 ...
 *        Adaptado para Raspberry Pi Pico (RP2040) usando el pin GP0.
 *
 * Nota: Este sketch está pensado para compilarse con el core de Arduino para RP2040.
 */

// --- Configuración de Pines ---
const int FSK_PIN = 0; // Raspberry Pi Pico: GP0

// --- Parámetros FSK ---
const int FREQ_0 = 1000;
const int FREQ_1 = 2000;
const long BIT_DURATION_US = 11000; // 11 ms por bit
...
const unsigned long PACKET_INTERVAL_MS = 0; // 0 = transmitir continuamente sin espera
...
void setup() {
	...
	Serial.println("Transmisor FSK con Checksum (RP2040) iniciado...");
...
void loop() {
	...
	if (current_time_us - bit_start_time_us >= BIT_DURATION_US) {
		...
		if (current_bit_index >= PACKET_LEN) {
			...
			digitalWrite(FSK_PIN, LOW);
			Serial.print("Paquete transmitido. Esperando ");
			Serial.print(PACKET_INTERVAL_MS);
			Serial.println(" ms para el siguiente.");
		}
	}
}
